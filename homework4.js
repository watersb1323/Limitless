...
function createDatePicker(month, year) {
	var tabObj, tabClassName = 'DP main_table';
...
function appendHeader(table, month, year) {
	...
	headerCell = addDOMClassName(headerCell, 'DP table_header');
...
function appendDayTitleRow(table) {
	...
	row = addDOMClassName(row, 'DP border_bottom');
}
...
function fillDates(table, month, year) {
	...
	while (currMonth === month) {
		...
		for (var i=0; i<=6; i++) {
			...
			if (currMonth !== month) {
				cell = addDOMClassName(cell,'DP faded_cell');
			}
			else {
				cell = addDOMClassName(cell,'DP normal_cell');
			}
